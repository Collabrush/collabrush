    ...
    <>
      {isLoading ? (
        ...
        <div className="min-h-screen min-w-screen bg-[#FFDE59] flex flex-col justify-center items-center noscroll">
          ...
          <div className="mt-16 text-3xl text-black">Your Collaboards</div>
          {/* <button
            type="button"
          ...
          <div className="my-8">
            {collaboards.length === 0 ? (
              ...
              <div className="grid min-[560px]:grid-cols-2 grid-cols-1 gap-4 md:grid-cols-3 min-[1240px]:grid-cols-5 lg:grid-cols-4 border-4 border-black bg-[#C7B9FF] p-4 mx-auto pb-96">
                <div
                  className="flex items-center justify-center w-56 px-6 py-3 space-x-4 space-y-4 text-base font-medium text-gray-900 bg-white border border-transparent rounded-lg shadow-sm h-44 group hover:bg-gray-300 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500"
                  onClick={async () => {
                    setIsCreatingCollaboard(true);
                    const { data: boards, error } = await supabase
                      .from("boards")
